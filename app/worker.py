...
from app import MARKDOWN_PATTERNS, config, db
...
def get_quote_from_db():
    try:
        quote = (
            ...
            .filter_by(id=random.randint(1, db.session.query(Quote).count()))
            .first()
        )
            ...
            file_list.append(
                {
                    ...
                    if round(file.size / 1024 / 1024, 2) > 0.0
                    else round(file.size / 1024, 2),
                    "size_unit": "MB"
                    if round(file.size / 1024 / 1024, 2) > 0.0
                    else "KB",
                }
